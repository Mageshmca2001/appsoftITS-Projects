...
namespace DLMS_MeterReading
{
    public class DLMS_AES
    {
        ...
        public static void Wrap()
        {
            ...
            // byte[] inputKek = new byte[] { 0x4E, 0x43, 0x43, 0x5F, 0x4D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x6B, 0x65, 0x79, 0x41, 0x6C, 0x6C }; // NCC_MasterkeyAll
            byte[] inputKek = new byte[] { 0x41, 0x45, 0x49, 0x5F, 0x4D, 0x61, 0x73, 0x74, 0x65, 0x72, 0x6B, 0x65, 0x79, 0x41, 0x6C, 0x6C }; // AEI_MasterkeyAll


            KeyParameter keyParam = new KeyParameter(inputKek);

            IBlockCipher symmetricBlockCipher = new AesEngine();

            Rfc3394WrapEngine wrapEngine = new Rfc3394WrapEngine(symmetricBlockCipher);

            byte[] inputData = new byte[] { 78, 67, 67, 95, 71, 108, 111, 98, 97, 108, 107, 101, 121, 95, 69, 65 };//NCC_Globalkey_EA
            wrapEngine.Init(true, keyParam);
            byte[] wrappedData = wrapEngine.Wrap(inputData, 0, inputData.Length);

            //byte[] wrapinputData = new byte[] { 0x83, 0xCC, 0xF8, 0xE9, 0x86, 0xBA, 0xFE, 0x70, 0x0B, 0xB4, 0x52, 0x5A, 0xCD, 0xB5, 0xF7, 0x03, 0x6C, 0xD7, 0x52, 0x5A, 0x92, 0x15, 0x67, 0x93 };//0000000000000001
            //wrapEngine.Init(false, keyParam);
            //byte[] unwrappedData = wrapEngine.Unwrap(wrapinputData, 0, 24);


            // Console.WriteLine("Wrapped Data: " + BitConverter.ToString(wrappedData).Replace("-", ""));
        }
