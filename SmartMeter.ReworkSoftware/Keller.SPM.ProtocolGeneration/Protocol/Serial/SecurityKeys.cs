...
namespace Keller.SPM.ProcotolGeneration.Protocol.Serial
{
    public class SecurityKeys
    {
        ...
        public byte[] PasswordKey { get; set; } = Encoding.UTF8.GetBytes(statusClass.GlobalKey);//("AEI_Globalkey_EA");
        public byte[] CNonce { get; set; } = Encoding.UTF8.GetBytes(statusClass.SystemTitle);//("AEI00001");
        public byte[] SecretKey { get; set; } = Encoding.UTF8.GetBytes(statusClass.HLS);//("AEI_SECRET_ASSC2");
